...
router.get('/login',(req,res)=>{
    ...
    //查询sql语句
    var sql="SELECT uid,user_name FROM tx_user WHERE uname=? AND upwd=md5(?)";
...
router.get('/reg',(req,res)=>{
    ...
    // console.log(obj)
    //判断账号是否已注册过.
   var sql="SELECT uid FROM tx_user WHERE uname=?"
    pool.query(sql,[obj.uname],(err,result)=>{
        ...
        if(result!=0){
           res.send({code:-1,msg:"账号已注册"})
            ...
            //账号未注册过,开始注册账号
            var reg_sql="INSERT INTO tx_user SET ?"
...
router.get("/person",(req,res)=>{
    ...
    if(!uid){
        ...
        var sql=`SELECT * FROM tx_user WHERE uid=${uid}`;
